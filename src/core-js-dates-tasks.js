...
function getNextFriday(date) {
  ...
  const delta = (5 - currentDay + 7) % 7;
  const result = new Date(date);

  result.setDate(date.getDate() + (delta === 0 ? 7 : delta));
...
function getWeekNumberByDate(date) {
  ...
  const days = (date.getTime() - year) / 1000 / 3600 / 24;
  const weekNumber = Math.ceil(days / 7) + 1;
  return year.getDay() === date.getDay() ? weekNumber : weekNumber - 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDate <= end) {
        schedule.push(format(currentDate));
